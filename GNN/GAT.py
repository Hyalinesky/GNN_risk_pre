...
class GAT(torch.nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, n_layers=2, dropout=0.4, heads=4):
        super(GAT, self).__init__()
        self.convs = torch.nn.ModuleList()
        self.dropout= dropout
        self.convs.append(GATConv(in_channels,hidden_channels, heads=4))#GATConv计算节点之间的注意力权重，最终输出更新后的节点表示
        self.convs.append(GATConv(hidden_channels*heads, out_channels))

    def forward(self, x, edges):
        for i, (edge_index, edge_type) in enumerate(edges):
...
# model = GAT(features=in_dim, hidden=args.h_dim, classes=out_dim, heads=4).to(device)
model = GAT(in_channels=in_dim, hidden_channels=args.h_dim, out_channels=out_dim, n_layers=2, dropout=0.4, heads=4).to(device)

if args.inference:
    model.load_state_dict(torch.load(osp.join("best_model", 'GAT-' + args.model_id + ".pth")))
else:
...
def train(epoch):
    ...
    for batch_size, n_id, adjs in train_loader:
        ...
        optimizer.zero_grad()
        edge_index = edges[0][0]
        # out, hidden = model(data.x[n_id].to(device), edge_index)[:batch_size]
...
def out_txt(list,name,epoch,savefig):
    ...
    if name == "loss":
        ...
        np.savetxt(savefig + "/GAT_loss_records.txt", out_list,fmt="%d %.20f")
    elif name == "acc":
        for l in x:
            line = [l, y[l - 1]]
            out_list.append(line)
        np.savetxt(savefig + "/GAT_acc_records.txt", out_list,fmt="%d %.20f")
...
if args.inference == False:
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        if args.validation and epoch >= args.early_stopping:
            ...
            #     torch.save(model.state_dict(), osp.join("best_model", 'GAT-'+args.model_id + ".pth"))
            #     best_epoch = epoch
